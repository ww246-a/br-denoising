...
def build_cnn_model(input_shape=(28, 28, 1), num_classes=10):
    model = keras.Sequential([
        ...
        keras.layers.Dropout(0.5),  # 添加Dropout

        keras.layers.Conv2D(16, (3, 3), activation='relu',  # 增加一层卷积
        ...
        keras.layers.Dense(32, activation='relu',
                          kernel_regularizer=keras.regularizers.l2(0.001)),
        keras.layers.Dropout(0.5),  # 全连接层后也加Dropout
...
# 构建模型
input_shape = (28, 28, 1)  # 灰度图像的形状
num_classes = 10  # MNIST 的类别数量
model = build_cnn_model(input_shape, num_classes)

# 训练模型
history = model.fit(X_train, y_train_processed, epochs=20, batch_size=128, validation_data=(X_test, y_test_processed))
...
def optimize_x_for_accuracy_exclude_x0(all_A1_t1_list, gray_values_list, y_train, y_test, step=0.02, max_x=5, target_accuracy=0.95):
    ...
    best_x = None  # 除 x=0 外最佳 x 值
    best_accuracy = 0  # 除 x=0 外最佳准确率
    accuracy_trend = []  # 用于记录每次迭代的准确率
    x_values = []  # 记录每次迭代的 x 值
    x0_accuracy = None  # 保存 x=0 的准确率
    x0_history = None   # 保存 x=0 的训练历史
    best_history = None
    ...
    while current_x < max_x:
        ...
        num_classes = 10
        model = build_cnn_model(input_shape, num_classes)
        history = model.fit(X_train, y_train_processed, epochs=20, batch_size=128, validation_data=(X_test, y_test_processed), verbose=0)
        ...
        if current_x != 0 and val_accuracy >= target_accuracy and val_accuracy > best_accuracy:
            ...
            best_x = current_x
            best_history = history.history  # 保存最佳 x 的训练历史

        # 保存最佳x的训练日志到txt文件
        output_file = f"D:\Zzj\output\curve_new2\Cnn_bestx_{current_x:.2f}.txt"
        ...
        # 新增：保存每次迭代的数据到txt文件
        iteration_output_file = "D:\Zzj\output\curve_new2\Cnn_x.txt"
...
# 保存图1的数据到 txt 文件
def save_plot1_data(x0_history, best_history, best_x, filename="D:\Zzj\output\curve_new2\Cnn_plot1_comparison_data.txt"):
